      ...
      -->
        ...
        >
          ...
          <el-tab-pane label="商品内容" name="4">
            ...
            <!-- 添加商品的按钮 -->
            <el-button type="primary" class="btnAdd" @click="addGoodsSubmit"
              >添加商品</el-button
            >
          </el-tab-pane>
        </el-tabs>
...
export default {
  ...
  methods: {
    ...
    handleRemove(file) {
      ...
      //fixme 需要学 findIndex
      const index = this.addGoodsForm.pics.findIndex((p) => p.pic == filePath)
    ...
    addGoodsSubmit() {
      //进行数据的校验
      this.$refs.addGoodsFormRef.validate(async(valid, obj) => {
        if (!valid) {
          let errotStr = ''
        ...
        const copyGoodsForm = _.cloneDeep(this.addGoodsForm);
        //将三级分类数组 转 字符串
        ...
        this.manyTableDateList.forEach(item=>{
          ...
          this.addGoodsForm.attrs.push(neweInfo);
        })
        //处理静态属性
        ...
        //复制
        copyGoodsForm.attrs = this.addGoodsForm.attrs
